...
public class EventoTest {
	...
	public void setUp() {
		this.evento1 = new Evento(0, CUATRODENOVIEMBRE, 1L);
		...
		this.evento2 = new Evento(1, CUATRODENOVIEMBRE.atTime(LocalTime.NOON), Duration.ofHours(3));
	...
	public void eventoEsMenorlAEventoQueEmpiezaIgualPeroTerminaDespues() {
		...
		assertEquals(-1, eventos.get(0).compareTo(eventos.get(1)));
		assertEquals(1, eventos.get(1).compareTo(eventos.get(0)));
	}

	@Test

	public void eventoEsMenorATareaQueVenceDespuesDeQueEsteComienze() {
		var t = new TareaPuntual(0, CUATRODENOVIEMBRE.atTime(LocalTime.NOON));
	...
	public void generarNEventosPosterioresLosGeneraCorrectamente() {
		...
		assertEquals(7, eventos.get(0).getInicio().getDayOfMonth());
	}
